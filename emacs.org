...
* version control
** magit
除了第一次外，不提示输入密码：git config --global credential.helper store
...
You can also press S (uppercase "s") to stage all files, C-u s to stage all untracked files. To stage a tracked/untracked directory, you simply use s. To stage multiple items (files/directories), you can mark a region and simply press s.
Magit is very interactive. Far better than the interactive staging in stock Git. The above is just a minor thing. With the command magit-log (press l then press another l in magit-status), you can interact with every commit:
