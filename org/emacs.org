...
** DONE match
smartparens
** DONE skip to match parentheses(forward/backward-sexp)
C-M-left/right
* DONE format code
Emacs 里对代码的格式化支持的非常好，可以选中一块代码 indent-region : Ctrl-Alt-\ (or TAB)对这块代码重新进行格式化.
To convert existing code that uses spaces to using tabs, use M-x tabify or reindent the entire file (C-x h C-M-\). 
* DONE moccur
